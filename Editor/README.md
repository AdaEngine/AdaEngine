...
Сгенерированный проект использует локальный SwiftPM-пакет `AdaEditor`. Для сборки и запуска редактора выберите схему `AdaEditor (Editor)`, которую Xcode создаёт для executable-продукта пакета.
