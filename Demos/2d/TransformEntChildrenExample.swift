...
struct TransformEntChildrenPlugin: Plugin {
    ...
    func setup(in app: borrowing AppWorlds) {
        app.main.addSystem(SetupTransformEntChildrenSystem.self, on: .startup)
        app.main.addSystem(ParentMovementSystem.self)
    }
}

@PlainSystem
struct SetupTransformEntChildrenSystem {
    init(world: World) { }
    ...
    func update(context: UpdateContext) async {
        ...
        do {
            ...
            characterAtlas = TextureAtlas(from: charactersTiles, size: [20, 23], margin: [4, 1])
        } catch {
            fatalError("Failed to load characters atlas: \(error)")
        }

        var camera = Camera()
        camera.backgroundColor = Color(135/255, 206/255, 235/255, 1)
        camera.clearFlags = .solid
        context.world.spawn(bundle: OrthographicCameraBundle(camera: camera))

        let parent = context.world.spawn("parent") {
            Transform(scale: Vector3(2), position: [0, 0, 0])
            Sprite(texture: characterAtlas[0, 0], size: Size(width: 56, height: 56))
            ParentComponent()
        }

        let child = context.world.spawn("child") {
