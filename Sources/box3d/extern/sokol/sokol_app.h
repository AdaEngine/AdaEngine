...
EM_JS(void, sapp_js_make_custom_mouse_cursor, (int cursor_slot_idx, int width, int height, const void* pixels_ptr, int hotspot_x, int hotspot_y), {
    ...
    const w32 = (val) => {
        bmp[index++] = val & 255;
        bmp[index++] = (val >> 8) & 255;
        bmp[index++] = (val >> 16) & 255;
        bmp[index++] = (val >> 24) & 255;
    };
    ...
    index += 64; // color space stuff, unused for 'Win ' or 'sRGB'
    assert(index == bmp_hdr_size + dib_hdr_size);
    const row_pitch = width * 4;
    for (let y = 0; y < height; y++) {
        const src_idx = pixels_ptr + y * row_pitch;
        const dst_idx = index + (height - y - 1) * row_pitch;
...
_SOKOL_PRIVATE void _sapp_win32_create_window(void) {
    ...
    */
    ...
    // Local patch: when high_dpi is requested, treat desc.width/desc.height as
    // physical/framebuffer pixels rather than 100%-DPI logical pixels.
    if (_sapp.desc.high_dpi) {
        rect.right = _sapp.window_width;
        rect.bottom = _sapp.window_height;
    } else {
...
_SOKOL_PRIVATE bool _sapp_android_touch_event(const AInputEvent* e) {
    ...
    int32_t index = action_idx >> AMOTION_EVENT_ACTION_POINTER_INDEX_SHIFT;
    _sapp_init_event(type);
